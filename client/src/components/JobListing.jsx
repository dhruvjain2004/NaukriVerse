        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center mt-8 gap-2 flex-wrap">
            ...
            {getPageNumbers()[getPageNumbers().length - 1] < totalPages && (
              <>
                {getPageNumbers()[getPageNumbers().length - 1] < totalPages - 1 && <span className="px-2 text-gray-500">...</span>}
                <button 
                  className="px-3 py-2 border border-gray-300 rounded hover:bg-gray-100 text-sm"
            ...
            <div className="w-full text-center text-gray-600 text-sm mt-2">
              Page {currentPage} of {totalPages} • Showing {paginatedJobs.length} of {filteredJobs.length} jobs
            </div>
          </div>
        )}
      </section>
